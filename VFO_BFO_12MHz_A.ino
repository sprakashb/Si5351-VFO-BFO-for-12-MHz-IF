...
// Used in hardwired circuit with multiband bitx or other board having similar LCD connection. Else Modify Lines 27-32 as per connectio
// Now also compiles with new 5351 library. 
//-----------------------------
...
#define ENCODER_A    2       // Encoder pin A on Mega - others D2
#define ENCODER_B    3       // Encoder pin B on Mega - others D3
...
volatile uint32_t if_offset = 00003200UL;      // in bitx board 5 1600 Hz difference in 40m
...
volatile uint32_t opfreq=0UL;       // Operating freq to be generated by Si5351 feeding to VFO input 
...
#ifdef IF_Offset
  if (vfo > 10000000UL)
    opfreq=vfo - bfo + if_offset;    // USB
  else
    opfreq= bfo - vfo + if_offset;  // LSB
 
  si5351.set_freq(opfreq, SI5351_CLK0);          // SI5351_PLL_FIXED did not give any output so 0 
  volatile uint32_t vfoT = opfreq;    
  tbfo = "LSB";
  // Now produce BFO
  // Use CLK2 output to generate bfo frequency
  si5351.set_freq( bfo, SI5351_CLK2); // spb
...
#ifdef IF_Offset
    if (vfo > 10000000UL)
      opfreq= vfo - bfo + if_offset;    // USB +if_offset to take care of diff in display
    else
     opfreq= bfo - vfo + if_offset;    // LSB
     
     // VFO
    si5351.set_freq(opfreq, SI5351_CLK0);         // SI5351_PLL_FIXED was not OK on 7MHz
    // BFO 
    si5351.set_freq( bfo , SI5351_CLK2); // spb
